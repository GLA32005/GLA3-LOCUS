...
mission:
  ...
  scope:
    - "example.com"          # 域名目标（系统启动时会自动解析为 IP 并加入白名单）
  oob: []                     # 无禁止目标
  risk_level: 3
  approved_ops:
    - "port_scan"
...
- `exploitation` — 漏洞利用 (msf/custom)
- `privilege_escalation` — 提权
- `lateral_movement` — 横向移动
- `http_probe` — HTTP 服务指纹探测 (httpx)
- `dir_enum` — 目录爆破与隐藏路径发现 (feroxbuster)
...
系统包含多项内置的工程优化，确保在复杂网络环境下的实战效果：
- **域名自动解析**：启动时自动将 scope 中的域名转换为 IP，并同步更新安全校验白名单，支持对 CDN 或云服务的透明探测。
- **403/502 启发式绕过**：当 Web 探测返回 403 Forbidden 时，系统会自动触发 `dir_enum` 并携带绕过字典（bypass profile），尝试寻找未授权路径。
- **任务去重冷却**：同一目标的相同探测任务在 30 分钟内不会重复执行。如果 Planner 重复下发，Orchestrator 会利用 Redis 锁自动拦截，节省 Token 和时间。
- **生命周期补偿**：当系统因侦察进入死胡同而强制切换到 Exploit 阶段时，如果剩余时间不足 20 分钟，会自动补偿生命周期，确保爆破任务能完整跑完。
- **工具别名映射**：针对 LLM 可能产生的“工具幻觉”，系统内置了映射层，自动将非标准工具名转换为现有的有效指令。
...
**Q: 任务显示“处于冷却期”被跳过？**
A: 这是系统的去重机制。相同的 (Target, Tool) 在 30 分钟内只执行一次。如果需要强制重扫，可以手动清除 Redis 中的 `sent_recon:*` 键。

**Q: 为什么强制进入了 EXPLOIT 阶段？**
A: 如果系统在 RECON 阶段连续 4 次（默认）没有获得任何有效的新资产或信息增益（Stall），Orchestrator 会认为侦察已触及边界，并强制引导 Planner 进入利用阶段尝试突破。
